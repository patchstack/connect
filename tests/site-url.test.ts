...
describe('normaliseSiteUrl', () => {
  ...
  it('refuses every address that names a network rather than a published site', () => {
    // The reported address is fetched by Patchstack later, so a host that resolves inside a network is
    // not merely wrong. `URL` canonicalises the alternative spellings before this sees them, so matching
    // the canonical form matches `0x7f.1`, `2130706433` and `[::ffff:127.0.0.1]` too.
    for (const host of [
      ...
      'http://0x7f.1',
      'http://2130706433',
      'https://203.0.113.10',                       // a public literal is still a literal
    ]) {
      expect(normaliseSiteUrl(host)).toBeNull();
    }
  });

  it('refuses a host that is only a name on somebody’s own network', () => {
    for (const host of [
      'https://production',
      'https://localhost.',
      'https://LOCALHOST',
  ...
  it('still accepts an ordinary published address', () => {
    for (const host of [
      'https://shop.example.com',
      'https://my-app-abc123.vercel.app',
      'https://internal-tools.acme.io',
      'https://example.com.au',
    ]) {
      expect(normaliseSiteUrl(host)).toBe(host);
    }
  });
  ...
  it('refuses an origin longer than Patchstack accepts', () => {
