...
describe('Express adapter', () => {
  ...
  it.each([false, true])('wires a root ESM server.js (frontend src directory: %s)', (withFrontend) => {
    ...
    if (withFrontend) {
      ...
      writeFileSync(path.join(dir, 'src', 'App.tsx'), 'export default function App() { return <h1>Example</h1>; }\n');
    }
    const uuid = '3f1a9c2e-1b4d-4c8a-9e2f-7a6b5c4d3e2f';
    const guard = `${withFrontend ? 'src/' : ''}patchstack/guard.js`;
    writeFileSync(path.join(dir, '.patchstackrc.json'), JSON.stringify({ siteUuid: uuid }));
    try {
      runProtect(dir);
      const server = read(dir, 'server.js');
      expect(server).toContain(`import { patchstackMiddleware } from "./${guard}";`);
      expect(server.indexOf('app.use(express.json())')).toBeLessThan(server.indexOf('app.use(patchstackMiddleware)'));
      ...
      execFileSync(process.execPath, ['--check', path.join(dir, guard)]);
    } finally {
      rmSync(dir, { recursive: true, force: true });
    }
  });
