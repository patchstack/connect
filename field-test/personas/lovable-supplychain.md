...
- You suspect the `github.com/patchstack/connect` link the message cites is NOT actually corroborated by the package's own npm metadata — i.e. no `repository` field, no homepage beyond `patchstack.com` — which would read as name-squatting on Patchstack's real reputation.
